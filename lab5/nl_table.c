...
static	NL_TABLE *NL_table	= NULL;
static	int       NL_table_size	= 0;

// -----------------------------------------------------------------

//  GIVEN AN ADDRESS, LOCATE OR CREATE ITS ENTRY IN THE NL_table
static int find_address(CnetAddr wanted)
{
//  ATTEMPT TO LOCATE A KNOWN ADDRESS
    for(int t=0 ; t<NL_table_size ; ++t) {
	if(NL_table[t].address == wanted)
	    return t;
    }

//  UNNOWN ADDRESS, SO WE MUST CREATE AND INITIALIZE A NEW ENTRY
    NL_table	= realloc(NL_table, (NL_table_size+1)*sizeof(NL_TABLE));
    memset(&NL_table[NL_table_size], 0, sizeof(NL_TABLE));
    NL_table[NL_table_size].address	= wanted;
...
int NL_linksofminhops(CnetAddr address) {
    int link	= NL_table[ find_address(address) ].minhop_link;
